...
const UserList=()=>{
    ...
    const url = 'https://randomuser.me/api/?nat=us&inc=name,picture,email&results=5&noinfo';
    ...
    const getUsers = async () => {
        ...
        const processedUsers = usersFetched.results.map( ({name, email, picture:{medium}})=>({ name : `${name.first} ${name.last}`, email, img: medium}));
    ...
    return(
        < div >
            ...
            <Suspense fallback={<div> {console.log("~hiiii")}Loading...</div>}>
             { users.length > 3 ? users.map((user, ind)=>(<UserInfoRow key={ind} {...user}/>)) : null}
            </Suspense>
